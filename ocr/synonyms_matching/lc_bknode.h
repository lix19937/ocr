...
//https://cloud.tencent.com/developer/article/1057868
//https://github.com/sholiday/cppBKTree
...
class BKNode {
    ...
    public:
      	inline BKNode(const T &);
        inline BKNode();
        inline BKNode(T, std::map<int, BKNode<T> >);
        inline ~BKNode();
        
        // Accessors
        inline	T get() const;

        inline int distance(const BKNode<std::string> &) const;
        inline	std::vector<T> find(const T &, const int&);
...
//  int dist;
//
//  bool operator <(const BKOuts &oth)const {
//    return dist < oth.dist;
//  }
//};
//
//template <typename T>
//std::map<T, int> bk_outs;
...
std::vector<T> BKNode<T>::find(const T &rhs, const int &threshold)  {
    ...
    //static std::map<T, int> bk_outs;
    // If the current node is in the threshold, return it
    if (dist <= threshold) {
      value.append("=");
      results.push_back(value.append(std::to_string(dist)));
      //std::cout << value << ", dist:" << dist << std::endl;
      //bk_outs.insert(std::make_pair(value, dist));
      //std::cout << "sss" << bk_outs.size()<< std::endl;
    }
